# YouTube Playback Position Auto Save v1.10.1
...
1. **Video detection**: When you open a YouTube video page, `content.js` detects the video ID and video element.
2. **Saved position check**: If a saved position exists, the extension checks the video duration before restoring it. Restoration is deferred while an ad is playing.
3. **Position restoration**: To restore a position, the extension pauses the video and seeks to the saved position. Playback then resumes if the corresponding setting is enabled.
4. **Active playback saving**: Periodic saves and event-triggered saves keep the playback position up to date.
5. **Completion handling**: A video is considered completed when it ends or reaches the final 3 seconds. If automatic deletion is enabled, its saved data is deleted. Otherwise, its ending position is saved if it meets the minimum save time.
...
### Suppression cases
- **During restoration and after completion**: Saving is suppressed during restoration and after completion to avoid overwriting the saved position incorrectly. If you seek back from the ending position after completing a video, saving can resume.
- **During ads**: Saving is also suppressed during ads so that an ad's playback position or duration is not stored as data for the main video.
...
### Restoration behavior during ads
...
### Playback Position Data (Key: `yt_position_<videoId>`)
- Video ID
- Playback position
- Video duration
- Save timestamp
...
### Settings Data (Key: `yt_position_settings`)
...
### Script Injection and Reinjection
...
- **Scope**: The extension works on YouTube video pages (`https://www.youtube.com/watch?v=...`).
- **During ads**: Ad playback positions are not saved; restoration of the main video is retried after ads finish.
- **Live streams**: Playback positions are neither saved nor restored for ongoing live streams, including DVR playback.
- **Autoplay restrictions**: Browser autoplay restrictions may prevent playback from resuming automatically after restoration. If this happens, start playback manually.
- **Duration changes**: If the video duration changes by more than 5 seconds, restoration is skipped to avoid restoring an incorrect position.
