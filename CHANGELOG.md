...
### 修正
- 広告/広告ブロッカー等で `duration` が一時的に不安定な場合でも、復元が誤ってスキップされにくいよう判定を安定待ち付きに変更

## [1.4.1] - 2025-12-14

### 修正
- 復元時に `seeked` が発火しない環境でも保存が停止し続けないよう、待機にタイムアウトと到達判定フォールバックを追加

## [1.4.0] - 2025-12-14

### 追加
- 設定画面（Options）を追加
- 保存間隔をプリセット＋カスタム（整数・1秒以上）から選択可能
- 通知表示のON/OFF
- 復元後の自動再生ON/OFF
- 広告中は保存/復元しない（実験的）
